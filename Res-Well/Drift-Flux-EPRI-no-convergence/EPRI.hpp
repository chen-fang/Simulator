...
{
   ADs tmp( 0.0 );
   tmp = pow( (-ReyNum_Liquid + gamma)/300000.0, 0.4 );
			...
			double     Dh,            const ADs& ReyNum_Liquid_dHg )
{
   ...
   ADs test2( 0.0 );
   test2 = 0.4 * pow( tmp, 0.4-1.0 ) / (-300000.0) * ReyNum_Liquid_dHg;
		    ...
		    const ADs& j_Liquid_CCFL, double     Dh )
{
   ...
   ADs ret( 0.0 );
   ret = -1.7 * pow( fabs(ReyNum_Liquid), 0.035 ) * exp( -fabs(ReyNum_Liquid)/( 35000.0*jfrx+25000.0 ) * c );
